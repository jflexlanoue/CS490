...
class grading {
    ...
    function DoGrading() {
       ...
       $this->FeedBack[] = "Answer and Solution Similarity: " . $MatchPerc . "%";
       ...
       $SolMaxGrade = $this->GetEstimateScore($this->SolutionTokens);
       $AnsMaxGrade = $this->GetEstimateScore($this->AnswerTokens);
       
       $EstMax = ($AnsMaxGrade / $SolMaxGrade ) ;
       ...
       $this->Result['Levenshetein'] = $this->GetLevenshteinEstimate();
       $this->FeedBack[] = "Estimated Levenshtein Cost: " . $this->Result['Levenshetein'];
    ...
    function CalculateScore(){
        ...
        $score += 25 * (($this->Result['StringMatching'] +  $this->Result['Completness']) /2) ;
    ...
    function GetEstimateScore($toks){
       $MaxScore = 0;
       ...
       foreach($toks as $T){
           ...
           switch($T['type']){
               case "Arithmetic":
                   $MaxScore += 5;  //Type Match
                   $MaxScore += 10; //Value Match
                   break;
               
               case "Relational":
                   $MaxScore += 10;  //Type Match
                   $MaxScore += 10; //Value Match
                   break;
